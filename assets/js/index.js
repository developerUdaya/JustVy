...
const transitionOffset = 925;

CustomEase.create("draw", "0.75, 0, 0.4, 1");
CustomEase.create("loading", "0.9, 0, 0.3, 1");
...
function pageTransitionIn() {
   ...
   tl.to(".loading-darker", {
      opacity: 1,
      duration: 0.6,
      ease: "Power1.easeIn",
   }, "<");

   tl.set(".loading-darker", {
      opacity: 0,
   });
...
function pageTransitionOut() {
   ...
   if(document.querySelector('.loading-fade-in-bottom')) {
      tl.set(".loading-fade-in-bottom", {
         opacity: 0,
         y: "2em"
      });
   }

   tl.to("header h1 .single-word-inner", {
      duration: 1.5,
      yPercent: 0,
      ease: "Expo.easeOut",
      stagger: .1,
   ...
   if(document.querySelector('header .mobile-hero .single-word-inner')) {
      tl.to("header .mobile-hero .single-word-inner", {
         duration: 1.5,
         yPercent: 0,
         ease: "draw",
         stagger: .1,
   ...
   tl.call(function() {
      initAnimateSinglePlainLoad();
      $('.news-single-header figure').addClass('loaded');
   }, null, 0);
...
function initPageTransitions() {
   ...
   function initResetDataAfter() {
      ...
      // $('[data-list-filter-status]').attr('data-list-filter-status', 'not-active');
      $('[data-list-search-status]').attr('data-list-search-status', 'not-active');
   }
...
/**
* Shape animation staggered
* With help from: https://codepen.io/GreenSock/pen/yLJxJZZ
*/

/* Start Staggered Animation on Load (Wrapped) */
function initAnimateSinglePlainLoad() {

   const plains = document.querySelectorAll("[data-plain-animate='load']");

   plains.forEach(plain => {
      const plainTimeline = gsap.timeline();
      const shapes = plain.querySelectorAll(".shape.negative");
      initAnimateSingleShape(plainTimeline, shapes);
   });
}

/* Start Staggered Animation on Load (Wrapped) */
function initAnimateSinglePlainScroll() {

   const plains = document.querySelectorAll("[data-plain-animate='scroll']");
...
/* Staggered Animation */
function initAnimateSingleShape(plainTimeline, shapes) {
   shapes.forEach((shape, i) => {
      if(shape.querySelector('.shape-side.top')) {
         plainTimeline.fromTo(shape.querySelectorAll(".shape-side.top"), {
            scaleY: 1,
         },{
            scaleY: 0,
            duration: shapeSideDuration,
            ease: shapeSideEase
         }, i * shapeSideStagger);
      }

      if(shape.querySelector('.shape-side.right')) {
         plainTimeline.fromTo(shape.querySelectorAll(".shape-side.right"), {
            scaleX: 1,
         },{
            scaleX: 0,
            duration: shapeSideDuration,
      ...
      if(shape.querySelector('.shape-side.bottom')) {
         plainTimeline.fromTo(shape.querySelectorAll(".shape-side.bottom"), {
            scaleY: 1,
         },{
            scaleY: 0,
            duration: shapeSideDuration,
      ...
      if(shape.querySelector('.shape-side.left')) {
         plainTimeline.fromTo(shape.querySelectorAll(".shape-side.left"), {
            scaleX: 1,
         },{
            scaleX: 0,
            duration: shapeSideDuration,
...
function initBasicFunctions() {
   ...
   // Toggle List Filter
   $('[data-list-filter-toggle="toggle"]').click(function(){
      if ($('[data-list-filter-status]').attr('data-list-filter-status') == 'not-active') {
         $('[data-list-filter-status]').attr('data-list-filter-status', 'active');
      } else {
         $('[data-list-filter-status]').attr('data-list-filter-status', 'not-active');
      }
   });
   ...
   // Toggle List Search
   $('[data-list-search-toggle="toggle"]').click(function(){
      if ($('[data-list-search-status]').attr('data-list-search-status') == 'not-active') {
         $('[data-list-search-status]').attr('data-list-search-status', 'active');
   ...
   // Toggle Contact Option (Mobile)
   $('.section-contact-information').each(function(){
      var sectionContactInfo = $(this);
      sectionContactInfo.find('.switch-contact-mode[data-contact-option]').click(function(){
         var dataContactOption = $(this).data('contact-option');
         if ($(this).attr('data-contact-option-status') == 'not-active') {
            $(this).siblings().attr('data-contact-option-status', 'not-active');
            $(this).attr('data-contact-option-status', 'active');
            sectionContactInfo.find('.single-contact-information[data-contact-option="' + dataContactOption + '"]').attr('data-contact-option-status', 'active');
            sectionContactInfo.find('.single-contact-information[data-contact-option="' + dataContactOption + '"]').siblings().attr('data-contact-option-status', 'not-active');
         }
      });
   });
}
...
function initAnimateSingleShape(plainTimeline, shapes) {
   ...
   const shapeSideDuration = 0.6;
   const shapeSideStagger = (shapeSideDuration * 0.725);
   const shapeSideEase = "draw";

   shapes.forEach((shape, i) => {

      // Enable Custom stagger
      i = shape.getAttribute('data-shape-stagger');
...
function initScrollTriggerParallaxScroll() {
   ...
   if(document.querySelector('.marquee .marquee-scroll')) {
      // Scrolltrigger Animation : Example
      $('.marquee .marquee-scroll').each(function (index) {
...
/**
* List JS (Sorting, filtering, searching)
*/
function initListJS() {

   // Source: https://listjs.com/
   // Filters: https://codepen.io/souls21998/pen/poELNVy
   // Pagination: https://jsfiddle.net/gwd7hcmy/13/

   $('#list-init').each(function () {

      var thisListID = $(this);
      var thisListPerPage = 16;

      if (thisListID.hasClass('variant-list-news')) {
         thisListPerPage = 6;
      }      
    
      // Note: category = tag->slug()
      var newList = new List('list-init', {
         valueNames: [
            'name', 
            'tag',
         ...
         thisListID.find(".pagination-prev.disabled, .pagination-next.disabled").removeClass("disabled");
         if (isFirst) {
            thisListID.find(".pagination-prev").addClass("disabled");
         }
         if (isLast) {
            thisListID.find(".pagination-next").addClass("disabled");
         }
       
         // Hide pagination if there one or less pages to show
         if (list.matchingItems.length <= thisListPerPage) {
            thisListID.find(".pagination").hide();
         } else {
            thisListID.find(".pagination").show();
         }

         // Add filter/search count of items
         thisListID.find('[data-filter-items-count').text(newList.matchingItems.length);
         ...
         newList.filter();
         thisListID.find('[data-filter-status]').attr('data-filter-status', 'not-active');
         thisListID.find('[data-filter-status-clear]').attr('data-filter-status-clear', 'not-active');
      });

      if(newList.matchingItems.length <= thisListPerPage) {
         thisListID.find(".pagination").hide();
         thisListID.find(".pagination-next").addClass("disabled");
      }

      // Previous Button
      thisListID.find(".pagination-prev").on('click', function(){
         thisListID.find(".pagination .active").prev().find(".page").trigger("click");
      });

      // Next Button
      thisListID.find('.pagination-next').on('click', function(){
         thisListID.find(".pagination .active").next().find(".page").trigger("click");
      });

      // Filter on Tag: [data-filter-status]
      thisListID.find('[data-filter-status]').on('click',function(){
         ...
         if($(this).attr('data-filter-status') == 'active'){
            ...
            newList.filter(function(item) {
               return (item.values().category.includes(listFilter));
            });
            $(this).attr('data-filter-status', 'active').siblings().attr('data-filter-status', 'not-active');
            thisListID.find('[data-filter-status-clear]').attr('data-filter-status-clear', 'active');
         }
      });


      // Filter remove All: [data-filter-status-clear]
      thisListID.find('[data-filter-status-clear]').on('click',function(){
         newList.filter();
         thisListID.find('[data-list-filter]').attr('data-filter-status', 'not-active');
         thisListID.find('[data-filter-status-clear]').attr('data-filter-status-clear', 'not-active');
         return false;
      });
